...
export async function getLiveKitToken(roomType: 'audition' | 'main_show', userName: string): Promise<string> {
  try {
...
export async function callUpContestant(queueId: string, roomId: string): Promise<{ user_id: string }> {
...
export async function endPerformance(roomId: string): Promise<void> {
  // This would need to be implemented as a Supabase function
  // For now, throw an error to indicate this needs to be implemented
